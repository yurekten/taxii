...
APP_TAXII_PROXY_DEPLOYMENT_YAML_FILE=k8s/app1_ns/taxii-proxy.yaml
...
kubectl create secret generic taxii-proxy-auth-json --from-file k8s/app1_ns/secrets/taxii-proxy-auth.json --namespace=$SERVER_NAMESPACE
kubectl create secret generic taxii-proxy-envs --from-env-file k8s/app1_ns/secrets/taxii-proxy-envs --namespace=$SERVER_NAMESPACE
